...
/* ── Niche-specific copy ── */
const TYPING_WORDS: string[] = ["sua marmitaria", "seu delivery de marmitas", "sua quentinha"];
const HERO_BADGE = "Usado por mais de 120 marmitarias";
const HERO_METRICS = [
  { value: "Até 3x", label: "mais pedidos fixos mensais" },
  { value: "Redução de 50%", label: "no tempo de separação" },
  { value: "Clientes fidelizados", label: "com plano mensal" },
];
const CTA_FINAL_TITLE = "Pronto para sua marmitaria ter clientes fixos todo mês?";
const PIXEL_CONTENT_NAME = "Landing Marmitaria";
...
const painPoints = [
  { icon: AlertTriangle, pain: "Você perde pedidos de marmita porque WhatsApp não organiza", solution: "Painel de pedidos organizados por horário de entrega e endereço" },
  { icon: Bot, pain: "Clientes esquecem de pedir e você perde receita recorrente", solution: "Robô IA faz cobrança automática e remarketing no WhatsApp" },
  { icon: Package, pain: "Controle de quantas marmitas fazer por dia é no papel", solution: "Painel mostra produção necessária do dia em tempo real" },
  { icon: Truck, pain: "Delivery por conta própria sem rota otimizada gera custo alto", solution: "Gestão de entregas com zonas e taxas configuráveis" },
];
...
const sellingModes = [
  { icon: Truck, title: "Delivery de marmitas", desc: "Pedidos no seu link, sem comissão. Painel organizado por horário e endereço.", cta: "Criar cardápio grátis" },
  { icon: Bot, title: "Robô IA + remarketing", desc: "Cobrança automática de clientes mensais e reativação no WhatsApp.", cta: "Começar agora" },
  { icon: Store, title: "Retirada no balcão", desc: "Marmita pronta para retirada na hora marcada pelo cliente.", cta: "Ver demonstração" },
  { icon: Package, title: "Produção do dia", desc: "Painel mostra a produção exata necessária para o dia. Zero desperdício.", cta: "Experimentar" },
];
...
const testimonials = [
  { name: "Maria J.", role: "Marmitas da Maria", text: "Tenho 200 clientes fixos de marmita mensal. O robô cobra automaticamente toda semana." },
  { name: "Carlos E.", role: "Quentinha Express", text: "A produção do dia aparece pronta no painel. Acabou o erro de fazer marmita a mais ou a menos." },
  { name: "Rosa F.", role: "Marmitas Caseiras", text: "Dobrei o número de clientes em 3 meses com o remarketing automático do WhatsApp." },
];

/* ── Component ── */
const LandingMarmitaria = () => {
  ...
  return (
    <div className="min-h-screen bg-card overflow-x-hidden font-sans">
      ...
      <section className="relative overflow-hidden bg-card">
        <div className="absolute inset-0 bg-gradient-to-b from-green-600/[0.08] via-green-600/[0.03] to-transparent" />
        <div className="absolute top-10 left-1/2 -translate-x-1/2 w-[900px] h-[900px] rounded-full bg-green-600/[0.10] blur-[120px] pointer-events-none" />

        <NicheHeroDecor
          bgPatternEmoji="🍱"
          keyframes={`
            @keyframes steam { 0%,100%{transform:translateY(0px); opacity:1} 50%{transform:translateY(-12px); opacity:0.8} } @keyframes steam-slow { 0%,100%{transform:translateY(0px) rotate(0)} 50%{transform:translateY(-18px) rotate(4deg)} }
          `}
          emojis={[
            { emoji: "🍱", className: "top-[8%] left-[6%] text-6xl sm:text-7xl [animation:steam_5s_ease-in-out_infinite]" },
            { emoji: "🥘", className: "top-[18%] right-[8%] text-5xl sm:text-6xl [animation:steam-slow_6s_ease-in-out_infinite]" },
            { emoji: "🥗", className: "bottom-[24%] left-[5%] hidden md:block text-5xl sm:text-6xl [animation:steam_7s_ease-in-out_infinite]" },
            { emoji: "🍲", className: "bottom-[12%] right-[6%] text-5xl sm:text-6xl [animation:steam-slow_5.5s_ease-in-out_infinite]" },
          ]}
        />

        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-12 sm:pt-24 sm:pb-16 text-center">
          <ScrollReveal>
            <NicheBadge bgClass="bg-green-600/15 text-green-800 border-green-600/30">
              🍱 {HERO_BADGE}
            </NicheBadge>
          </ScrollReveal>
      ...
      <NicheExtraSection
        ...
        cards={[
          { emoji: "📝", title: "Cliente assina plano", desc: "Pacotes de 5, 10 ou 20 marmitas com desconto. Cliente escolhe e fecha pelo celular." },
          { emoji: "🤖", title: "Robô cobra automático", desc: "Cobrança recorrente todo mês via Pix ou cartão. Você não corre atrás de ninguém." },
          { emoji: "🚚", title: "Marmita entregue, fidelidade construída", desc: "Cardápio da semana atualizado em segundos pelo celular. Cliente sabe o que vai chegar." },
        ]}
      />
      ...
      {/* ═══ CTA FINAL ═══ */}
      <section className="py-16 sm:py-20 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #16A34A 0%, #14532D 100%)' }}>
...
export default LandingMarmitaria;
