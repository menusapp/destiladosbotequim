...
-- ---------------------------------------------------------------------
-- No Lovable Cloud não temos acesso ao JWT Secret do projeto, então não dá
-- para emitir um JWT que o PostgREST valide como `authenticated`. Solução:
-- sessão no servidor — o login cria um token OPACO (uuid aleatório) numa
-- tabela; o app envia esse token no header `x-app-token`; os helpers do RLS
-- validam o token na tabela e liberam o acesso. Sem segredo, sem edge
-- function de assinatura. Seguro: o token é aleatório e não forjável.
-- =====================================================================

-- 1) Tabela de sessões (só definer/service_role acessam — RLS sem policies)
...
-- 2) Leitor do token vindo do header da requisição (PostgREST expõe headers).
...
-- 3) Helpers do RLS — agora resolvem identidade pela sessão do header.
...
-- Helpers precisam ser executáveis por anon (o papel da requisição continua
-- anon; a identidade vem do token no header, não do papel do Postgres).
...
-- 4) Login → cria a sessão e devolve o token.
...
-- 5) As policies de staff foram criadas como `TO authenticated`, mas agora a
--    requisição continua no papel `anon` (a identidade vem do token). Então
--    fazemos as policies valerem para TODOS os papéis — o gate real é o
--    `current_restaurant_id()`/`is_ceo()` dentro do USING/WITH CHECK.
--    (Anon sem token → helper retorna NULL → acesso negado.)
DO $$
DECLARE
  r record;
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION WHEN others THEN
      -- ignora policies que não permitem alteração de role
      NULL;
    END;
  END LOOP;
END $$;
